...
def main():
    ...
    a = subprocess.Popen(f'PLAYERKEY={attacker_player_key} APIKEY={api_key} AI_NAME={os.getenv("ATTACKER_AI_NAME")} python local_main.py > ../logs/attacker_log_{now} 2> ../logs/attacker_err_log_{now}', shell=True)
    d = subprocess.Popen(f'PLAYERKEY={defender_player_key} APIKEY={api_key} AI_NAME={os.getenv("DEFENDER_AI_NAME")} python local_main.py > ../logs/defender_log_{now} 2> ../logs/defender_err_log_{now}', shell=True)
    a.communicate()
    d.communicate()
