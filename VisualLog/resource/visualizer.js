...
Main.main = function() {
	...
	window.setInterval(Main.onEnterFrame,16.666666666666668);
};
...
log_LogReaderBase.prototype = {
	...
	,readRotation: function(points) {
		var max = Math.PI * 2 / UInt.toFloat(points);
	...
	,readBool: function() {
		...
		default:
			var value = _g;
			throw haxe_Exception.thrown("unknown bool: " + value);
		}
	}
	,skip: function(data) {
...
var trlog_TrlogReader = function(reader) {
	...
	if(name != 1869378164) {
		var tmp = 1869378164;
		throw haxe_Exception.thrown("visual log identifer must be " + (tmp == null ? "null" : Std.string(UInt.toFloat(tmp))) + " but " + (name == null ? "null" : Std.string(UInt.toFloat(name))));
	}
	var version = reader.input.readInt32();
	if(version != 1) {
		throw haxe_Exception.thrown("unsupported version: " + (name == null ? "null" : Std.string(UInt.toFloat(name))));
	}
	var headerSize = reader.input.readInt32();
	if(headerSize < 16) {
		throw haxe_Exception.thrown("unsupported header size: " + headerSize);
	}
		...
		while(_g < _g1.length) {
			...
			while(_g2 < _g3) {
				...
				while(this.position < target) {
					var _g4 = this.readByte();
					if(_g4 == 0) {
						var string = this.readLongString();
						stringBuf_b += string == null ? "null" : "" + string;
					} else {
						var command = _g4;
						throw haxe_Exception.thrown("unknow command: " + command);
					}
				}
			}
		}
...
var vilog_BlendMode = {};
vilog_BlendMode._new = function(value) {
	if(value < 0 || 16 < value) {
		throw haxe_Exception.thrown("unknown blend mode: " + value);
	}
	var this1 = value;
	return this1;
};
var vilog_TextureRequired = {};
vilog_TextureRequired._new = function(value) {
	if(value != 0 && value != 1 && value != 2) {
		throw haxe_Exception.thrown("unsuppoted TextureRequired: " + value);
	}
	var this1 = value;
	return this1;
};
var vilog_VilogCommandKind = {};
vilog_VilogCommandKind._new = function(byte) {
	var this1 = byte;
	return this1;
};
...
var vilog_VilogReader = function(reader) {
	...
	this._textureRequired = vilog_TextureRequired._new(reader.input.readByte());
	this._frameSize = reader.input.readInt32();
	this._entrySize = reader.input.readInt32();
	this._byteSize = reader.input.readInt32();
...
vilog_VilogReader.prototype = $extend(log_LogReaderBase.prototype,{
	...
	,proceedCommands: function(drawLayer,begin,length) {
		...
		while(UInt.gt(end,this.position)) {
			var _g = vilog_VilogCommandKind._new(this.readByte());
			switch(_g) {
			case 0:
			...
			case 24:
				...
				break;
			case 25:
				drawLayer.setBlendMode(vilog_BlendMode._new(this.readByte()));
				break;
			case 26:
			...
			default:
				var value = _g;
				throw haxe_Exception.thrown("unsupported visual command: " + value + " position:" + this.position);
			}
		}
	}
...
var visualizer_Visualizer = function(element) {
	...
	var _g = 0;
	var _g1 = element.getElementsByClassName("trace");
	while(_g < _g1.length) {
		var child = _g1[_g];
		++_g;
		var src = child.getAttribute("src");
		if(src != null) {
			targets.push(new visualizer_state_VisualizerContent(child,src,visualizer_ContentKind.Trace));
			traceViews.push(new visualizer_trace_TraceView(child));
		}
	}
...
var visualizer_VisualizerView = function(visualizer,root,traceViews,visualViews) {
	this.root = root;
	...
	this.controlView = new visualizer_ctrl_VisualControlView(visualizer,content1);
};
...
visualizer_VisualizerView.prototype = {
	draw: function() {
		...
		case 0:
			...
			var _g1 = 0;
			var _g2 = this.traceViews;
			while(_g1 < _g2.length) {
				var traceView = _g2[_g1];
				++_g1;
				traceView.clear();
			}
			var _g1 = 0;
			var _g2 = this.visualViews;
			while(_g1 < _g2.length) {
				var visualView = _g2[_g1];
				++_g1;
				visualView.clear();
			}
			break;
		...
		case 2:
			...
			var _g = 0;
			var _g1 = this.traceViews.length;
			while(_g < _g1) {
				var index = _g++;
				var traceView = this.traceViews[index];
				traceView.draw(state.traceReaders[index],state.maxHeight,state.get_step());
			}
			var _g = 0;
			var _g1 = this.visualViews.length;
			while(_g < _g1) {
				var index = _g++;
				var visualView = this.visualViews[index];
				visualView.draw(state.visualReaders[index],state.get_step());
			}
			break;
		}
	}
};
var visualizer_ctrl_VisualControlView = function(visualizer,container) {
	this.visualizer = visualizer;
	this.step = -1;
	this.frameSize = 0;
	this.playing = false;
	...
	var tmp = util_ElementUtil.span({ "class" : "icon-sb_playback_play"},"");
	var tmp2 = this.startButton = util_ElementUtil.element("button",{ "class" : "big", disabled : "disabled"},[tmp]);
	var tmp = util_ElementUtil.span({ "class" : "icon-sb_playback_fastforward"},"");
	var tmp3 = this.forwardButton = util_ElementUtil.element("button",{ "class" : "small", disabled : "disabled"},[tmp]);
	var tmp = this.frameRange = util_ElementUtil.element("input",{ "class" : "frame-range", type : "range", value : "0", min : "0", max : "0", step : "1", disabled : "disabled"},[]);
	var tmp4 = util_ElementUtil.span({ "class" : "icon-sb_arrow_d_left"},"");
	var tmp5 = this.fastBackstepButton = util_ElementUtil.element("button",{ "class" : "small", disabled : "disabled"},[tmp4]);
	var tmp4 = util_ElementUtil.span({ "class" : "icon-sb_arrow_s_left"},"");
	var tmp6 = this.backstepButton = util_ElementUtil.element("button",{ "class" : "small", disabled : "disabled"},[tmp4]);
	var tmp4 = this.frameText = util_ElementUtil.span({ "class" : "frame-text"},"");
	var tmp7 = util_ElementUtil.span({ "class" : "icon-sb_arrow_s_right"},"");
	var tmp8 = this.stepButton = util_ElementUtil.element("button",{ "class" : "small", disabled : "disabled"},[tmp7]);
	var tmp7 = util_ElementUtil.span({ "class" : "icon-sb_arrow_d_right"},"");
	var tmp9 = this.fastStepButton = util_ElementUtil.element("button",{ "class" : "small", disabled : "disabled"},[tmp7]);
	container.appendChild(util_ElementUtil.div({ "class" : "row"},[tmp1,tmp2,tmp3,tmp,tmp5,tmp6,tmp4,tmp8,tmp9]));
...
var visualizer_state_VisualizerDisplayState = function(visualReaders,traceReaders) {
	...
	while(_g < traceReaders.length) {
		...
		if(UInt.gt(reader.get_frameSize(),a)) {
			this.frameSize = reader.get_frameSize();
		}
	}
	var _g = 0;
	while(_g < visualReaders.length) {
		var reader = visualReaders[_g];
		++_g;
		this.fps = reader.config.fps;
		if(this.maxHeight < reader.config.height) {
			this.maxHeight = reader.config.height;
		}
		var a = this.frameSize;
		if(UInt.gt(reader.get_frameSize(),a)) {
			this.frameSize = reader.get_frameSize();
...
var visualizer_state_VisualizerLoadingState = function(visualizer,targets) {
	...
	this.states = _g;
	var _g = 0;
	var _g1 = targets.length;
	while(_g < _g1) {
		var index = _g++;
		var target = targets[index];
		this.startLoad(target.src,target.kind,index);
	}
};
	...
	,drawBackground: function(argb) {
		if(this.previousBackground != argb) {
			...
			if(alphaChannel != 255) {
				...
				this.background.beginFill(rgb,UInt.toFloat(alphaChannel) / UInt.toFloat(255));
...
visualizer_visual_VilogLayerDrawView.prototype = {
	getGraphics: function() {
		...
		case 1:
			...
			this.currentState = visualizer_visual__$VilogLayerDrawView_VisualLayerDrawViewState.Graphics(graphics);
			...
			this.matrix.copy(graphics.localTransform);
			this.add(graphics);
			return graphics;
		}
	}
	...
	,drawRegularPolygon: function(x,y,points,radius,rotation) {
		...
		var delta = Math.PI * 2 / UInt.toFloat(points);
		var polygon = [];
		var _g = 0;
		var _g1 = points;
		while(_g < _g1) {
	...
	,drawGrid: function(x,y,gridWidth,gridHeight,cellWidth,cellHeight) {
		var width = UInt.toFloat(gridWidth) * cellWidth;
		var height = UInt.toFloat(gridHeight) * cellHeight;
		...
		while(_g < _g1) {
			var i = _g++;
			this.moveTo(x,y + cellHeight * i);
			this.lineTo(x + width,y + cellHeight * i);
		}
	}
	...
	,arrowTo: function(toX,toY,tailVisible,bodyVisible,headVisible,arrowWidth,arrowLength) {
		...
		if(tailVisible) {
			...
			var x = arrowLength;
			var y = -arrowWidth / 2;
			this.lineTo(from.x + x * Math.cos(r) - y * Math.sin(r),from.y + x * Math.sin(r) + y * Math.cos(r));
		}
		if(headVisible) {
			var r = Math.atan2(toY - from.y,toX - from.x);
			var x = -arrowLength;
			var y = arrowWidth / 2;
			this.moveTo(toX + x * Math.cos(r) - y * Math.sin(r),toY + x * Math.sin(r) + y * Math.cos(r));
			this.lineTo(toX,toY);
			var x = -arrowLength;
			var y = -arrowWidth / 2;
			this.lineTo(toX + x * Math.cos(r) - y * Math.sin(r),toY + x * Math.sin(r) + y * Math.cos(r));
		}
	}
	...
	,arcArrow: function(ctrlX,ctrlY,radius,startAngle,endAngle,anticlockwise,tailVisible,bodyVisible,headVisible,arrowWidth,arrowLength) {
		...
		if(tailVisible) {
			...
			var x = arrowLength;
			var y = -arrowWidth / 2;
			this.lineTo(baseX + x * Math.cos(r) - y * Math.sin(r),baseY + x * Math.sin(r) + y * Math.cos(r));
		}
		if(headVisible) {
			var r = endAngle + Math.PI / 2 * (anticlockwise ? 1 : -1);
...
vilog_BlendMode.Normal = 0;
vilog_BlendMode.Add = 1;
vilog_BlendMode.Multiply = 2;
vilog_BlendMode.Screen = 3;
vilog_BlendMode.Overlay = 4;
vilog_BlendMode.Darken = 5;
vilog_BlendMode.Lighten = 6;
vilog_BlendMode.ColorDodge = 7;
vilog_BlendMode.ColorBurn = 8;
vilog_BlendMode.HardLight = 9;
vilog_BlendMode.SoftLight = 10;
vilog_BlendMode.Difference = 11;
vilog_BlendMode.Exclusion = 12;
vilog_BlendMode.Hue = 13;
vilog_BlendMode.Saturation = 14;
vilog_BlendMode.Color = 15;
vilog_BlendMode.Luminosity = 16;
vilog_TextureRequired.No = 0;
vilog_TextureRequired.Yes = 1;
vilog_TextureRequired.Unknown = 2;
vilog_VilogCommandKind.LineTo = 0;
vilog_VilogCommandKind.MoveTo = 1;
vilog_VilogCommandKind.QuaraticCurveTo = 2;
vilog_VilogCommandKind.BezierCurveTo = 3;
vilog_VilogCommandKind.Arc = 4;
vilog_VilogCommandKind.ArcTo = 5;
vilog_VilogCommandKind.Fill = 6;
vilog_VilogCommandKind.ClosePath = 7;
vilog_VilogCommandKind.DrawCircle = 8;
vilog_VilogCommandKind.DrawEllipse = 9;
vilog_VilogCommandKind.DrawRectangle = 10;
vilog_VilogCommandKind.DrawRoundRectagle = 11;
vilog_VilogCommandKind.DrawRegularPolygon = 12;
vilog_VilogCommandKind.DrawStar = 13;
vilog_VilogCommandKind.DrawGrid = 14;
vilog_VilogCommandKind.DrawColumns = 15;
vilog_VilogCommandKind.MultiplyTranform = 16;
vilog_VilogCommandKind.ResetTranform = 17;
vilog_VilogCommandKind.MultiplyAlpha = 18;
vilog_VilogCommandKind.ResetAlpha = 19;
vilog_VilogCommandKind.SetLineAlpha = 20;
vilog_VilogCommandKind.SetFillAlpha = 21;
vilog_VilogCommandKind.SetLineColor = 22;
vilog_VilogCommandKind.SetFillColor = 23;
vilog_VilogCommandKind.SetLineThickness = 24;
vilog_VilogCommandKind.SetBlendMode = 25;
vilog_VilogCommandKind.SetLineAlignment = 26;
vilog_VilogCommandKind.SetFonts = 27;
vilog_VilogCommandKind.SetFontSize = 28;
vilog_VilogCommandKind.SetFontStyle = 29;
vilog_VilogCommandKind.SetTextHolizontalAlign = 30;
vilog_VilogCommandKind.SetTextVerticalAlign = 31;
vilog_VilogCommandKind.DrawText = 32;
vilog_VilogCommandKind.SetImageHolizontalAlign = 33;
vilog_VilogCommandKind.SetImageVerticalAlign = 34;
vilog_VilogCommandKind.DrawImage = 35;
vilog_VilogCommandKind.StartMaskingRegion = 36;
vilog_VilogCommandKind.StartMaskedRegion = 37;
vilog_VilogCommandKind.EndMaskRegion = 38;
vilog_VilogCommandKind.ReferenceCommands_16_8 = 39;
vilog_VilogCommandKind.ReferenceCommands_24_16 = 40;
vilog_VilogCommandKind.ReferenceCommands_32_24 = 41;
vilog_VilogCommandKind.ArrowTo = 42;
vilog_VilogCommandKind.CurveArrowTo = 43;
vilog_VilogCommandKind.ArcArrow = 44;
Main.main();
